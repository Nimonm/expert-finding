...
# 加载用户
memb_temp = pd.read_csv(f'{base_path}/member_info_0926.txt', header=None, sep='\t')
memb_temp.columns = ['uid', 'sex', 'keyw', 'mun_layer', 'hot_layer', 'reg_type', 'reg_platform', 'freq', 'ba',
                'bb', 'bc', 'bd', 'be', 'salt_value', 'focus_topic', 'ua', 'ub', 'uc', 'ud', 'ue', 'like_topic']
# , 'focus_topic', 'like_topic'
memb = memb_temp[['uid', 'mun_layer', 'hot_layer', 'freq', 'salt_value', 'ba','bb', 'bc', 'bd', 'be']]
...
fea_cols = ['is_good', 'is_rec', 'is_dest', 'has_img', 'has_video', 'word_count',
            ...
            'mun_layer', 'hot_layer', 'salt_value', 'ba','bb', 'bc', 'bd', 'be']
...
def extract_feature1(target, label_feature, ans_feature):
    ...
    # 回答部分特征
    # ['aid', 'qid', 'uid', 'ans_dt', 'ans_t1', 'ans_t2', 'is_good', 'is_rec', 'is_dest', 'has_img',
    #  'has_video', 'word_count', 'reci_cheer', 'reci_uncheer', 'reci_comment', 'reci_mark', 'reci_tks',
    #  'reci_xxx', 'reci_no_help', 'reci_dis']
...
model_lgb = LGBMClassifier(boosting_type='gbdt', num_leaves=48, learning_rate=0.05, n_estimators=2000,
                           max_bin=255, subsample_for_bin=50000, objective='binary', min_split_gain=0,
                           min_child_weight=5, min_child_samples=10, subsample=0.8, subsample_freq=1,
                           colsample_bytree=1, reg_alpha=3, reg_lambda=5, seed=321, n_jobs=1, silent=True)
